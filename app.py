...
def sumit():
    corpus=polarity()
    word_text=text_data_cleaning(str(corpus))
    pd.set_option("display.max_colwidth", 200)
    def freq_words(x, terms = 30):
      all_words = ' '.join([text for text in x])
      all_words = word_text.split()
    ...
    def freq_words(x, terms = 30):
      ...
      return d
    d=freq_words(word_text)
    output_file('plot.html')
    b=figure(x_range= list(d['word']), plot_height=500, title="most_frequent_words",
           toolbar_location=None, tools="")
    b.vbar(
         x= list(d['word']),
        top= list(d['count']),
    ...
    b.y_range.start = 0
    show(b)
    sent=sumit.groupby('sentiment')
    positive_values=sent.get_group('Positive')
    negative_values=sent.get_group('negative')
    
    negative_text=text_data_cleaning(str(negative_values['Review']))
    
    freq_words(negative_text)
    
    positive_text=text_data_cleaning(str(positive_values['Review']))
    
    freq_words(positive_text)
